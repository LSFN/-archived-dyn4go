...
func newNode(comparable Comparable) *node {
	return newNode(comparable, nil, nil, nil)
}
...
func (n *node) CompareTo(other node) int {
	return n.comparable.CompareTo(other)
}
...
func (iter *treeIterator) next() *node {
	result := iter.currentNode
	if iter.currentNode != nil {
		if iter.isAscending {
			if iter.currentNode.left != nil {
				// First preference is the left node
				iter.currentNode = iter.currentNode.left
			} else if iter.currentNode.right != nil {
				// Second preference is the right node
				iter.currentNode = iter.currentNode.right
			} else {
				// Otherwise traverse back up the tree and then down again if possible
				if iter.currentNode.parent == nil {
...
/* This method traverses the tree in the "ascending" order.
 * The left subtree is traversed first, then the right subtree, then the node itself
